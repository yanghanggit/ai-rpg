...
#######################################################################################################################################
# 战斗公式常量 - 已移至系统提示词 FANTASY_WORLD_RPG_GLOBAL_GAME_MECHANICS
# COMBAT_FORMULA_RULES: Final[
#     str
# ] = """**战斗公式**
#
# **1. 伤害计算（攻击者A → 目标B）**
...
# B.当前HP = B.当前HP - 总伤害
# ```
# → 若 B.当前HP ≤ 0：检查复活机制→有则恢复至MAX_HP并说明，无则标记死亡
#
# **2. 治疗计算（施法者A → 目标B）**
# ```
# 治疗量 = ceil(A.魔攻) + B.持续治疗
# B.当前HP = min(B.MAX_HP, B.当前HP + 治疗量)
# ```
#
# **3. 计算规则**
# - ceil() = 向上取整
...
```json
{{
  "combat_log": "【角色使用卡牌 → 环境互动(含数值) → 伤害结果(实时更新HP) → 卡牌代价】 → 最后所有角色最终HP(角色.HP=X/Y) → 环境更新。所有效果明确数值 → 精简紧凑文本",
  "narrative": "将战斗过程故事化简短概括：角色行动→环境响应→影响结果→更新后的环境状态。禁用数字，使用感官描写，只描述核心部分，减少细节描写，精简紧凑文本"
}}
```

**约束规则**
- combat_log压缩优化： 角色名简写(仅保留最后一段) → 卡牌仅写名称 → 环境互动仅写[物体名+效果] → 计算公式省略中间步骤直接写结果数值 → 状态/代价合并同类项用简写[名称(效果+数值)] → 删除所有连接词、修饰词、解释性文字 → 仅记录数值变化，不重复完整公式 → 环境更新用简短关键词描述核心变化
- **所有攻击必定命中目标，游戏中不存在影响攻击是否命中或被闪避的任何机制，禁止在combat_log和narrative中生成与此相关的效果、状态或代价**
