...
- 是否携带词缀及落点——无附加效果则不携带词缀；有附加效果则落在即时词缀（仅本次结算生效）。

数值参考一律挂靠出牌者聚合后的属性（如"伤害以本角色攻击力为基数"），不在 keyword 里硬编码绝对值。牌库生成 LLM 拿到的是确定的字段落点与相对量级，而非需要二次解释的模糊措辞。
...
**一效果一条，不分质量档。** 同一效果只保留一条普通档 keyword，不再并列"优质"变体。强度差异改由字段关系（如"低于攻击力"）在单条 keyword 内表达，而不是靠同一效果的多条质量变体由采样碰运气。
...
keyword 直接声明词缀字段与文本：有附加效果的 keyword 声明即时词缀（仅本次结算生效）。词缀文本（`[名称]:触发倾向描述`）由 keyword 给出，字段 schema 约束其格式。
...
- `GenerateDeckActionSystem` 消费 keywords 产出具体 `Card` → 参见：[LLM 生成型游戏对象：Card](llm-generated-objects.md)
