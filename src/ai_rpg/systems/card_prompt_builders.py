...
| affixes | 延迟词缀列表；**每个元素必须是单个字符串**（禁止输出 `{{"name": ..., "description": ...}}` 这样的对象），格式为 `名称:触发倾向描述`，例如 `"呛咳:呼吸不畅，下回合手牌伤害-2"`；出牌后独立推理生成持续状态效果；凡涉及"下回合/持续N回合/未来生效"等跨回合影响的效果（作用于目标，增减益与敌我无必然绑定，增益/减益均可、可兼有；如致盲、压制、防御提升、护盾等）必须放在这里；无则 [] |
...
| target_type | 含义 |
|---|---|
| `{TargetType.SINGLE}` | 作用于单个存活角色（伤害/治疗/增减益由卡牌效果自行决定，默认） |
| `{TargetType.ALL}` | 选定一个目标作为阵营锚点，实际作用于其所在阵营的全部存活角色（伤害/治疗/增减益由卡牌效果自行决定） |
