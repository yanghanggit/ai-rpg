...
class DrawCardsResponse(BaseModel):
    ...
    status_effects: List[StatusEffect] = Field(
        ...,
        description="你自身的状态效果列表，注意！场景，角色，已发生事件均可能对你产生影响并生成状态效果！",
    )
...
- 第一局一定会有新增的status_effects，根据角色进入战斗时的环境，内心活动，和其他角色的情况生成，而不是技能里提到的状态
...
- 禁用换行/空行
- 请严格按照示例格式输出合规JSON
- 输出格式要求response_sample中的任何数字都不是正确的值，请根据你行动结束后的状态生成正确的值"""
...
### 特殊规则
根据最近的[发生事件！战斗回合]中的”计算流程“结果来更新你的生命值，update_hp应当是你事件更新后的生命值。
...
- 禁用换行/空行
- 请严格按照示例格式输出合规JSON
- 输出格式要求response_sample中的任何数字都不是正确值，请根据你行动结束后的状态生成正确的值"""
