...
**驱动因子**：关键词（`DeckComponent.keywords`）+ 骰值（0–100 随机）。

`DeckComponent.keywords` 是角色的**完整牌库蓝图**——设计者在此声明该角色可能拥有的全部卡牌风格（如 3 张攻击、2 张防御、1 张破甲）。每场战斗实际生成几张牌，由 `get_cards_per_combat(entity)` 根据角色类型（PartyMember / Monster）决定；系统从 keywords 池中随机采样对应数量，逐条配以骰值后交由 LLM 创作具体卡牌。
