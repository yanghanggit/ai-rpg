...
def create_stage_cave1() -> Stage:
    ...
    return create_stage(
        ...
        stage_profile="你是一个阴暗潮湿的洞窟，墙壁上是各种血迹和断掉的剑，洞内还有哥布林的营地，地上散落着哥布林的粪便和食物残渣。洞穴深处传来低语声和偶尔的金属碰撞声，似乎有哥布林在进行某种活动。",
        actors=[],
    )
...
def create_demo_dungeon1() -> Dungeon:
    ...
    actor_goblin.kick_off_message += f"""\n注意:你非常狡猾，在遇到危险时会使用你的传家宝哥布林祭司的项链进入濒死状态，并在敌人放松警惕时以百分之十的血量复活并偷袭反击。但是这个项链只能让你复活一次。项链属于装备，不会在卡牌中出现，死亡时会自动触发，不受负面效果影响，不占用行动回合。"""
