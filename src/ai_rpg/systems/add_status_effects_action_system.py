...
| phase | 触发时机 | 典型效果 |
|---|---|---|
| `{PhaseType.DRAW}` | 本回合抽牌时 | 「虚弱」生成的卡牌伤害偏低 |
| `{PhaseType.ARBITRATION}` | 每次出牌结算时 | 「破甲」防御降低；「荆棘」反伤；条件计数型词条（`counter` 字段） |
| `{PhaseType.ROUND_END}` | 每回合末自动 tick | 「中毒」扣血；「燃烧」持续伤害；「再生」回血（DOT/HOT） |"""
...
- `phase`：`{PhaseType.DRAW}` / `{PhaseType.ARBITRATION}` / `{PhaseType.ROUND_END}` 三选一（见上表）
