...
from ..models.messages import HumanMessage, SystemMessage
...
class CombatInitStageSystem(ExecuteProcessor):
    ...
    async def execute(self) -> None:
        ...
        # 在核心 system prompt（[0]）之后插入战斗专用规则 system message（[1]），不用删除，因为是战斗场景在地下城结束后会被销毁，战斗专用规则 system message 也会随之销毁
        self._game.insert_messages(
            ...
            messages=[
                SystemMessage(
                    ...
                    combat_system_rules=current_stage_entity.name,
                )
            ],
        )
