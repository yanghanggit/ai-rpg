...
class CombatRoomScreen(BaseGameScreen):
    ...
    def __init__(self) -> None:
        ...
        self._selected_item_name: Optional[str] = (
            None  # 已选消耗品名（SELECT_CONSUMABLE_TARGET 跨步骤）
        )
        self._consumable_actor: Optional[str] = (
            None  # 使用消耗品的角色（SELECT_CONSUMABLE / SELECT_CONSUMABLE_TARGET）
        )
    ...
    async def _handle_consumable_selection(self, raw: str) -> None:
        ...
        log = self.query_one(RichLog)
        candidates = self._target_candidates  # item name list
        ...
        # 需要再拉一次实体信息以获取 target_type 和场上角色
        try:
