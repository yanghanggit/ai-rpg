...
###############################################################################################################################################
def _classify_faction(entity: Optional[EntitySerialization]) -> str:
...
###############################################################################################################################################
def _is_alive(entity: EntitySerialization) -> bool:
...
###############################################################################################################################################
def _compute_effective_stats_for(
...
class CombatPlayCardsScreen(BaseGameScreen):
    ...
    def _write_actor_detail(
        ...
        """
        effective_stats = _compute_effective_stats_for(entity)
    ...
    async def _fetch_state(self) -> Tuple[bool, str]:
        ...
        effective_stats = (
            _compute_effective_stats_for(current_entity)
            if current_entity is not None
            else None
        )
    ...
    def _enter_select_target(self, log: RichLog) -> None:
        ...
        actor_faction = _classify_faction(actor_entity)
        ...
        if card.target_type == TargetType.ALLY_ALL:
            self._flow.pending_targets = [
                ...
                and _classify_faction(entity) == actor_faction
                and _is_alive(entity)
            ]
            self._enter_confirm(log)
            return
        ...
        if card.target_type == TargetType.ENEMY_SINGLE:
            candidates = [
                (name, entity)
                for name, entity in self._snapshot.entities_map.items()
                if _classify_faction(entity) not in ("unknown", actor_faction)
                and _is_alive(entity)
            ]
