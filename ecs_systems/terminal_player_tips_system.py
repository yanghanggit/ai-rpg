...
############################################################################################################
    def tips_next_stages(self, player_proxy: PlayerProxy, player_entity: Entity) -> None:
        ...
        stage_names: List[str] = [self.parse_stage_name(stage_name, actor_name) for stage_name in cast(Set[str], stage_graph_comp.stage_graph) ]
        player_proxy.add_stage_message(self._context.safe_get_entity_name(stage_entity), 
                                       f"可去往场景:\n{'\n'.join(stage_names)}")
############################################################################################################
...
############################################################################################################
    def tip_stage_archives(self, player_proxy: PlayerProxy, player_entity: Entity) -> None:
        ...
        actor_name = self._context.safe_get_entity_name(player_entity)
        stage_archives = self._context._file_system.get_files(StageArchiveFile, actor_name)
        stage_names: List[str] = [stage_archive.name for stage_archive in stage_archives]
        
        player_proxy.add_actor_message(actor_name, f"已知场景:\n{'\n'.join(stage_names)}")
############################################################################################################
