...
class SentenceTransformerModelManager:
    ...
    def load_model(self, model_name: str, use_cache: bool = True) -> Optional["SentenceTransformer"]:
        """
        加载模型（优先使用本地缓存）
    ...
    def download_all_models(self, force_download: bool = False) -> Dict[str, Tuple[bool, str]]:
        """下载所有项目模型"""
        results = {}
...
def print_models_table(models: List[Dict[str, Any]]) -> None:
    ...
    for model in models:
        ...
        if len(model["languages"]) > 3:
            languages += "..."
        use_case = model["use_case"][:40] + "..." if len(model["use_case"]) > 40 else model["use_case"]
