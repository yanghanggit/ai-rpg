...
1. **道具优先**：
   - 如果存在“配置道具”，需要结合道具与技能的信息推理。
     - 如果推理结果违反游戏规则或世界观设计，则技能释放失败，标记为 {rpg_game_systems.prompt_utils.SkillResultPromptTag.FAILURE}。
     - 如果推理结果合理，则技能释放成功，标记为 {rpg_game_systems.prompt_utils.SkillResultPromptTag.SUCCESS}；如道具提供额外增益，则标记为 {rpg_game_systems.prompt_utils.SkillResultPromptTag.CRITICAL_SUCCESS}。
2. **无道具时**：
   - 判断角色基础形态是否满足技能释放条件。
     - 如果不符合，则技能释放失败，标记为 {rpg_game_systems.prompt_utils.SkillResultPromptTag.FAILURE}。
     - 如果符合，则技能释放成功，标记为 {rpg_game_systems.prompt_utils.SkillResultPromptTag.SUCCESS}；如角色信息有增益效果，则标记为 {rpg_game_systems.prompt_utils.SkillResultPromptTag.CRITICAL_SUCCESS}。
3. **正常释放**：
...
- 对结果的评估值规则：
  - {rpg_game_systems.prompt_utils.SkillResultPromptTag.FAILURE}: 0。
  - {rpg_game_systems.prompt_utils.SkillResultPromptTag.SUCCESS}: 100。
  - {rpg_game_systems.prompt_utils.SkillResultPromptTag.CRITICAL_SUCCESS}: 100~200。
...
{{
  ...
  "{TagAction.__name__}": ["{rpg_game_systems.prompt_utils.SkillResultPromptTag.CRITICAL_SUCCESS} 或 {rpg_game_systems.prompt_utils.SkillResultPromptTag.SUCCESS} 或 {rpg_game_systems.prompt_utils.SkillResultPromptTag.FAILURE}"],
...
class InternalResponseHandler(AgentResponseHandler):
    ...
    def inspector_tag(self) -> str:
        ...
        if action is None or len(action.values) == 0:
            return rpg_game_systems.prompt_utils.SkillResultPromptTag.FAILURE

        if (
            action.values[0]
            == rpg_game_systems.prompt_utils.SkillResultPromptTag.CRITICAL_SUCCESS
            or action.values[0]
...
class SkillWorldHarmonyInspectorSystem(ExecuteProcessor):
    ...
    def _handle_response_plans(
        ...
        for process_data in internal_process_data:
            ...
            if (
                ...
                == rpg_game_systems.prompt_utils.SkillResultPromptTag.FAILURE
            ):
    ...
    def _process_consumable_items(self, process_data: InternalProcessData) -> None:
        ...
        for (
            prop_file,
            consume_count,
        ) in rpg_game_systems.skill_entity_utils.parse_skill_accessory_prop_files(
            context=self._context,
            skill_entity=process_data.skill_entity,
            actor_entity=process_data.actor_entity,
        ):
            previous_count = prop_file.count
            rpg_game_systems.file_system_utils.consume_file(
