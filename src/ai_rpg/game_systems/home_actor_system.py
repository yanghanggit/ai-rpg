...
def _generate_prompt(
    ...
    response_sample = ActorResponse(
        ...
        query_actions="你要说的内容。判断逻辑：1) 如果在本次对话中有人向你提问，且你在本次的speak_actions中回复了“让我想想”，则这次的query_actions必须是根据上下文和这次的提问来决定如何发问从而深度思考自己的记忆去获取相关的信息和答案。2）如果没有最新的提问，则根据对话和上下文来决定是否发问以查询对话中提到的信息。注意：如果发问，则只关注其他角色本次的提问和对话，对于之前的提问，严禁再次思考。（只有你自己能听见，这个行动会触发记忆查询）",
    )
