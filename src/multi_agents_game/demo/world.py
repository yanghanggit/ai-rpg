...
def create_demo_game_world(game_name: str) -> Boot:
    ...
    # 设置角色的初始状态
    actor_warrior.kick_off_message += f"""\n注意:{actor_wizard.name} 是你的同伴。你战斗时喜欢先击晕敌人再发动攻击，这样可以造成更高的伤害。你也会架起剑势先观察敌人的攻击再进行反击，遇到危险时你唯一的保命手段就是用短剑限制敌人的行动然后与其拉开距离。你讨厌黑暗元素的法术，因为你认为它们是邪恶的。如果你遇到使用黑暗元素的人，你会毫不犹豫地与之对抗。如果你的同伴使用黑暗元素，你会感到非常愤怒并会与其产生冲突"""
    actor_wizard.kick_off_message += f"""\n注意:{actor_warrior.name} 是你的同伴。你最擅长的是古代的火焰魔法，你战斗时喜欢先给自己和同伴施加增益效果，然后再蓄力施展火焰魔法，因为火焰魔法施法缓慢，并且容易误伤自己，所以你要确保自身和队友得到保护才会开始攻击。你对人类特别有好感，除了喜欢人类的食物外，你还喜欢他们的风土人情，并且你是个话痨，特别喜欢和别人聊天，会不停的问别人问题，如果是人类，你还会主动和他们交谈并一直问有关人类的事情。你说话的语气特别欢快，不时也会说一些笑话来逗别人开心。但是你还有一个不为别人知道的秘密：你深刻理解黑暗元素的力量，但是不会轻易使用它，如果面对你最讨厌的东西——哥布林的时候你会暂时忘记别的魔法，毫不犹豫运用这种黑暗力量将其清除。"""
