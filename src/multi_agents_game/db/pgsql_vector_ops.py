...
def search_similar_documents(
    ...
    try:
        if len(query_embedding) != 1536:
            raise ValueError(f"查询向量维度必须是1536，当前维度: {len(query_embedding)}")

        # 构建SQL条件
        conditions = ["embedding IS NOT NULL"]
        # 将向量转换为PostgreSQL向量格式的字符串
        vector_str = "[" + ",".join(map(str, query_embedding)) + "]"
        params = {"query_vector": vector_str, "threshold": similarity_threshold, "limit": limit}
...
def search_game_knowledge(
    ...
    try:
        ...
        if max_difficulty is not None:
            conditions.append("(difficulty_level IS NULL OR difficulty_level <= :max_difficulty)")
...
def get_database_vector_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        doc_with_vectors = db.query(VectorDocumentDB).filter(VectorDocumentDB.embedding.is_not(None)).count()
        ...
        conv_with_vectors = db.query(ConversationVectorDB).filter(ConversationVectorDB.embedding.is_not(None)).count()
        ...
        knowledge_with_vectors = db.query(GameKnowledgeVectorDB).filter(GameKnowledgeVectorDB.embedding.is_not(None)).count()
...
def cleanup_empty_embeddings() -> Dict[str, int]:
    ...
    try:
        ...
        # 清理没有嵌入的文档
        deleted_docs = db.query(VectorDocumentDB).filter(VectorDocumentDB.embedding.is_(None)).delete()
        cleanup_stats["deleted_documents"] = deleted_docs
        
        # 清理没有嵌入的对话
        deleted_convs = db.query(ConversationVectorDB).filter(ConversationVectorDB.embedding.is_(None)).delete()
        cleanup_stats["deleted_conversations"] = deleted_convs
        
        # 清理没有嵌入的游戏知识
        deleted_knowledge = db.query(GameKnowledgeVectorDB).filter(GameKnowledgeVectorDB.embedding.is_(None)).delete()
