...
def router_node(state: UnifiedState) -> Dict[str, Any]:
    ...
    try:
        ...
        rag_keywords = [
            # 地名和世界设定
            "艾尔法尼亚", "阿斯特拉王国", "月桂森林联邦", "铁爪部族联盟",
            "封印之塔", "贤者之塔", "水晶城", "暗影墓地", "星辰神殿",
            
            # 重要物品和角色
            "圣剑", "晨曦之刃", "魔王", "玛拉凯斯", "黯蚀之主", "勇者", "莉莉丝",
            
            # 种族和职业
            "精灵", "兽人", "龙族", "矮人", "冒险者", "骑士", "法师", "战士",
            
            # 魔法和技能
            "火焰", "冰霜", "雷电", "治愈", "暗影", "净化之光", "审判之炎", "希望守护",
            
            # 组织和物品
            "冒险者公会", "暴风雪团", "时之沙漏", "生命之泉", "星辰钢", "魔法药水",
            
            # 通用游戏术语
            "王国", "联邦", "部族", "遗迹", "地下城", "魔法", "技能", "装备", "等级"
        ]
        
        # 检查关键词匹配
        query_lower = user_query.lower()
        matched_keywords = [keyword for keyword in rag_keywords if keyword in query_lower]
        ...
        if matched_keywords:
            ...
            logger.success(f"🚦 [ROUTER] 选择RAG模式，匹配到 {len(matched_keywords)} 个关键词")
        else:
        ...
        logger.info(f"🚦 [ROUTER] 路由决策完成: {route_decision} (置信度: {confidence_score:.2f})")
...
def enhancement_node(state: UnifiedState) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug(f"📝 [ENHANCEMENT] 增强后的上下文长度: {len(enhanced_context)} 字符")
...
############################################################################################################
def create_unified_chat_graph() -> CompiledStateGraph[UnifiedState, Any, UnifiedState, UnifiedState]:
    """
    创建统一的聊天图
...
def stream_unified_graph_updates(
    unified_compiled_graph: CompiledStateGraph[UnifiedState, Any, UnifiedState, UnifiedState],
