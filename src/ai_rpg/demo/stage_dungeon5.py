...
def create_demo_dungeon5() -> Dungeon:
    ...
    actor_training_robot.inventory.items.append(
        Item(
            ...
            description="此道具让你拥有无限生命! 即：每个战斗回合在生成技能与更新状态时，都会将生命恢复到你的Max_HP。如果你还没有'钢铁之心'相关的status_effect，则在第一次更新状态时创建一个，标记此道具的存在，duration=999。后续回合系统会自动维护此状态效果，无需重复创建。而且在战斗中如果你的生命值降到0，你不会死掉，而是会周身冒烟并坚挺地站立着，等待下一个回合的开始。",
        )
    )
