...
def setup_database_tables() -> Any:
    """设置数据库表的 fixture"""
    try:
        from src.multi_agents_game.pgsql.pgsql_client import (
...
def test_vector_document_operations() -> None:
    """测试向量文档操作 - 使用ORM"""
    from src.multi_agents_game.pgsql.pgsql_vector_document import (
...
def run_all_vector_tests() -> None:
    ...
    try:
        # 确保数据库表已创建
        from src.multi_agents_game.pgsql.pgsql_client import engine
        from src.multi_agents_game.pgsql.pgsql_client import Base  # type: ignore[attr-defined]
