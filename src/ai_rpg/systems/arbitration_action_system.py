...
**卡牌**封装完整行动信息（使用者、目标、攻防、描述、机制、词条）。结算规则：

1. **按序执行**：依照行动顺序逐张结算
2. **先手影响后手**：先执行的卡牌可改变环境、状态，影响后续卡牌
3. **死亡中断**：角色HP归零后，该角色对应的所有卡牌立即作废
4. **特殊情况**：复活、反击、同归于尽等特殊机制由【机制】段落或词条声明

{"\n\n".join(card_sequence_prompt)}

## 战斗导演职责

1. **严格执行词条与机制**：卡牌词条和【机制】段落声明的规则必须严格执行
2. **强制死亡中断**：角色HP归零后，该角色所有未执行卡牌立即作废，不记录任何效果

可在combat_log中标注环境变化（如地形破坏、天气转变等），这些环境变化将影响后续回合

## 输出格式

```json
{{
  "combat_log": "多行字符串，每行=一张卡牌执行",
  "final_hp": {{}},
  "narrative": "战斗演出描述"
}}
```
...
**格式**：`[简名|卡牌→目标:简短效果] HP:全部角色状态`

**要点**：
- 逐行记录，HP变化用→
- 死亡角色的未执行卡牌不记录执行过程
- 可选：环境互动`→环境物→`、代价`代价:具体描述`
- 最后可选环境总结：`[环境]关键词1+关键词2`
