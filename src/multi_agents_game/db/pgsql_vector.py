...
class VectorDocumentDB(UUIDBase):
    ...
    doc_metadata: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON字符串存储额外信息
    ...
    # 为向量字段创建索引以优化相似度搜索
    __table_args__ = (
        Index('ix_vector_documents_embedding', 'embedding', postgresql_using='ivfflat', postgresql_with={'lists': 100}),
...
class ConversationVectorDB(UUIDBase):
    ...
    # 对话时间戳
    message_timestamp: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    ...
    # 索引优化
    __table_args__ = (
        Index('ix_conversation_vectors_embedding', 'embedding', postgresql_using='ivfflat', postgresql_with={'lists': 100}),
...
class GameKnowledgeVectorDB(UUIDBase):
    ...
    # 索引优化
    __table_args__ = (
        Index('ix_game_knowledge_embedding', 'embedding', postgresql_using='ivfflat', postgresql_with={'lists': 100}),
