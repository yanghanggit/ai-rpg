...
**只写设计意图与时效语义，不写机制字段。** keyword 声明卡牌类型与效果的时效（破甲、持续侵蚀、持续一回合的防御），但不指定机制实现。字段落点由宪法「词缀（affix）」与牌库生成提示词的字段 schema 决定，跨回合效果由 LLM 自行落到 affixes。
...
keyword 声明的卡牌类型决定该卡是否携带词缀：攻击型不带，防御 / 控制 / 侵蚀型带延迟词缀。但 keyword 只表达设计意图，具体落到 affixes，由 LLM 依据宪法词缀规则在字段 schema 内完成。
