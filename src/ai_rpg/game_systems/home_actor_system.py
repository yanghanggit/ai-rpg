...
def _generate_prompt(
    ...
    response_sample = ActorResponse(
        speak_actions={
            "场景内角色全名": "你要说的内容。1）如果是回答问题，根据自己的上下文中已有的信息来进行答复。2）如果上下文中没有关于这个问题的信息，就回答“让我想想”。3）如果上一次speak_actions里已经出现了“让我想想”，那么这次speak_actions必须是直接回答问题，不能再出现“让我想想”。并且这次回答问题时，如果上下文中有和问题相关的信息就直接回答，如果上下文中还是没有相关信息就将所有搜到的信息复述出来。注意：只关注本次的提问，对于之前的提问，不需要再次回答。（场景内其他角色会听见）",
        },
        whisper_actions={
            "场景内角色全名": "你要说的内容（只有你和目标角色能听见）",
        },
        announce_actions="你要说的内容（所有的角色都能听见）",
        mind_voice_actions="你要说的内容。判断逻辑：1) 如果在本次对话中有人向你提问，且你在本次的speak_actions中回复了“让我想想”，则这次的mind_voice_actions只能生成这个最新的问题；2) 如果没有最新的提问或已经给出了确定答案，则生成你的内心独白。注意：只关注本次的提问，对于之前的提问，不需要再次回答。（只有你自己能听见）",
    )
