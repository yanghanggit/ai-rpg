...
装备是战斗中可主动使用的**持久性**道具，使用时消耗被装备目标本回合 1 点 `energy`。使用后以深拷贝写入目标角色的 `EquippedGearComponent`，持续提供属性加成直至关卡结束；原物品始终保留在 `InventoryComponent` 中，不被消耗。
...
每次为目标角色装备（含首次装备与替换旧装备）消耗其当前回合 1 点 `energy`（`RoundStatsComponent.energy`）；目标 `energy` 已耗尽（<=0）时无法为其装备。
...
- **写入**：战斗中使用装备时，以深拷贝写入目标角色；已激活或目标 energy 不足则服务层拒绝。
