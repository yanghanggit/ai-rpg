...
以对话消息列表的 SHA-256 hash 为 key，将单条 ChatMessage 序列化后
...
def compute_cache_key(
    ...
    """计算对话消息列表的 SHA-256 hash，作为缓存文件名。"""
    raw = json.dumps(
...
###########################################################################################################################################
def load_debug_cache(cache_key: str) -> Optional[ChatMessage]:
    """从磁盘加载缓存的 ChatMessage。"""
...
###########################################################################################################################################
def save_debug_cache(cache_key: str, message: ChatMessage) -> None:
    """将 ChatMessage 序列化后写入磁盘缓存。"""
