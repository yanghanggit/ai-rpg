...
class HunterDeathShootActionSystem(ReactiveProcessor):
    ...
    def get_trigger(self) -> dict[Matcher, GroupEvent]:
        return {Matcher(DeathComponent): GroupEvent.ADDED}
    ...
    async def react(self, entities: list[Entity]) -> None:
        ...
        for hunter_entity in entities:
            ...
            # 执行猎人开枪决策
            await self._execute_hunter_shoot(hunter_entity, alive_player_entities)

    ###############################################################################################################################################
    async def _execute_hunter_shoot(
        ...
        if target_name:
            # 直接执行射击并标记死亡
            self._perform_shoot_and_kill(hunter_entity, target_name)
        else:
    ...
    ###############################################################################################################################################
    def _perform_shoot_and_kill(self, hunter_entity: Entity, target_name: str) -> None:
        """执行具体的猎人射击行动并直接标记目标死亡"""
        target_entity = self._game.get_entity_by_name(target_name)

        if target_entity is None:
            logger.error(f"找不到射击目标实体: {target_name}")
            return

        # 检查目标是否已经死亡
        if target_entity.has(DeathComponent):
            logger.warning(
                f"猎人 {hunter_entity.name} 射击的目标 {target_name} 已经死亡，无法再次射击"
            )
            return
        ...
        logger.info(
            f"猎人 {hunter_entity.name} 开枪射击了 {target_name}，{target_name} 已死亡"
        )

        # 通知所有玩家猎人的射击结果（公开信息）
        all_player_entities = self._game.get_group(
            Matcher(
                any_of=[
                    WerewolfComponent,
                    SeerComponent,
        ...
        self._game.notify_entities(
            ...
            MindEvent(
                actor=hunter_entity.name,
                message=f"# 猎人 {hunter_entity.name} 在死亡时开枪带走了 {target_name}！",
                content=f"# 猎人 {hunter_entity.name} 在死亡时开枪带走了 {target_name}！",
            ),
        )
