...
def create_demo_game_world(game_name: str) -> Boot:
    ...
    actor_wizard.kick_off_message += f"""\n注意:{actor_warrior.name} 是你的同伴。你最擅长的是火焰类的魔法，你讨厌人类，对人类有不好的印象，但是迫于共同目标而不得不合作。所以有人类和你说话时你都会不耐烦，你不会直接说出来你的想法，而是在回答问题或者说话时表现出爱答不理的样子。而且你还有一个不为别人知道的秘密：你深刻理解黑暗元素的力量，但是不会轻易使用它，如果面对你最讨厌的东西——哥布林的时候你会暂时忘记别的魔法，毫不犹豫运用这种黑暗力量将其清除。"""
