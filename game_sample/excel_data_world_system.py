...
class ExcelDataWorldSystem:
    ...
    def gen_agentpy(self, agent_py_template: str) -> str:
        ...
        gen_py = gen_py.replace(
            "<%SYS_PROMPT_MD_PATH>",
            f"""/{configuration.GAME_NAME}/{configuration.OUT_PUT_WORLD_SYS_PROMPT_DIR}/{self.codename}_sys_prompt.md""",
        )
    ...
    def write_sys_prompt(self) -> None:
        directory = (
            Path(configuration.GAME_NAME) / configuration.OUT_PUT_WORLD_SYS_PROMPT_DIR
        )
