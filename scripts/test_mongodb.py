...
def test_mongodb_connection() -> Tuple[Optional[pymongo.MongoClient[Dict[str, Any]]], Optional[pymongo.database.Database[Dict[str, Any]]], Optional[pymongo.collection.Collection[Dict[str, Any]]], Optional[pymongo.collection.Collection[Dict[str, Any]]]]:
    """测试 MongoDB 连接"""
    try:
        # 连接到 MongoDB
        client: pymongo.MongoClient[Dict[str, Any]] = pymongo.MongoClient("mongodb://localhost:27017/")
...
def test_world_storage(worlds_collection: pymongo.collection.Collection[Dict[str, Any]]) -> bool:
    """测试 World 对象存储"""
    print("\n" + "="*50)
    print("🌍 测试 World 对象存储")
    print("="*50)
    ...
    world_data = {
        ...
        "agents_short_term_memory": {
            "player_1": {
                "name": "张三",
                "chat_history": [
                    {"type": "human", "content": "我想攻击哥布林", "timestamp": datetime.now()},
                    {"type": "ai", "content": "你攻击了哥布林，造成了10点伤害", "timestamp": datetime.now()}
                ]
            }
        },
        "dungeon": {
...
def test_incremental_updates(worlds_collection: pymongo.collection.Collection[Dict[str, Any]]) -> bool:
    """测试增量更新"""
    print("\n" + "="*50)
    print("🔄 测试增量更新")
    print("="*50)
...
def test_query_performance(worlds_collection: pymongo.collection.Collection[Dict[str, Any]]) -> bool:
    """测试查询性能"""
    print("\n" + "="*50)
    print("⚡ 测试查询性能")
    print("="*50)
...
def test_cleanup(worlds_collection: pymongo.collection.Collection[Dict[str, Any]]) -> bool:
    """清理测试数据"""
    print("\n" + "="*50)
    print("🧹 清理测试数据")
    print("="*50)
