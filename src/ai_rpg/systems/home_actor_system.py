...
def _generate_prompt(
    ...
    response_sample = ActorResponse(
        ...
        speak_actions={
            "角色全名": "说话内容。检索时填表达'需要思考'含义的简短话语,根据角色性格和说话风格进行填写,不超过10字。收到检索结果后必须回答",
        },
        whisper_actions={
            "角色全名": "耳语内容(其他人听不到)。检索时填表达'需要思考'含义的简短话语,根据角色性格和说话风格进行填写,不超过10字。收到检索结果后必须回答",
        },
        announce_actions="公开宣布内容(所有人听到)。检索时填表达'需要思考'含义的简短话语,根据角色性格和说话风格进行填写,不超过10字。收到检索结果后必须回答",
...
2. 事实性提问必须检索且仅检索一次:
   ① 首次遇到→填写检索关键词→说思考用语→等待
   ② 收到检索结果后→必须立即回答(基于结果或说'我不知道')→检索字段必须留空
   ③ 已检索过的问题→绝不可再次检索(即使结果不满意)
