...
class TestMcpTools:
    ...
    def test_create_sample_mcp_tools(self, sample_tools):
        """测试 MCP 工具创建功能"""
        # 验证工具数量
        ...
        assert all(name in tool_names for name in expected_names), f"工具名称不匹配: {tool_names}"
    ...
    def test_get_current_time_tool(self, sample_tools):
        """测试时间查询工具"""
    ...
    def test_calculator_tool_basic_operations(self, sample_tools):
        """测试计算器工具的基本运算"""
        test_cases = [
            ("2+3", "计算结果：5"),
            ("10-4", "计算结果：6"),
        ...
        for expression, expected in test_cases:
            result = execute_mcp_tool("calculator", {"expression": expression}, sample_tools)
            assert result == expected, f"表达式 {expression} 计算错误: 期望 {expected}, 实际 {result}"
    
    def test_calculator_tool_error_handling(self, sample_tools):
        """测试计算器工具的错误处理"""
        # 测试非法字符
        result = execute_mcp_tool("calculator", {"expression": "2+3+import"}, sample_tools)
    ...
    def test_text_processor_tool_operations(self, sample_tools):
        """测试文本处理工具的各种操作"""
        ...
        for operation, expected in test_cases:
            result = execute_mcp_tool("text_processor", {
                "text": test_text, 
                "operation": operation
            }, sample_tools)
            assert result == expected, f"文本操作 {operation} 错误: 期望 {expected}, 实际 {result}"
    
    def test_text_processor_tool_invalid_operation(self, sample_tools):
        """测试文本处理工具的无效操作处理"""
        result = execute_mcp_tool("text_processor", {
            "text": "test", 
    ...
    def test_execute_mcp_tool_nonexistent_tool(self, sample_tools):
        """测试执行不存在的工具"""
        result = execute_mcp_tool("nonexistent_tool", {}, sample_tools)
        assert "未找到" in result, "应该返回工具未找到的错误信息"
    
    def test_execute_mcp_tool_error_handling(self, sample_tools):
        """测试工具执行的错误处理"""
        # 测试缺少必要参数
...
class TestMcpState:
    ...
    def test_mcp_state_creation(self):
        ...
        # 验证工具数量
        assert len(state["tools_available"]) == 3, f"应该有 3 个可用工具，实际: {len(state['tools_available'])}"
    
    def test_mcp_state_disabled_tools(self):
        """测试禁用工具的 MCP 状态"""
        state: McpState = {
            "messages": [],
            "tools_available": [],
