...
词缀为自由文本 `[名称]:触发倾向描述`（如 `[穿透]:本次伤害无视目标防御`），描述该牌在对应时机结算的行为。
...
| `{TargetType.ALL}` | 阵营锚点：以指定目标为锚点，作用于其所在阵营（远征队/怪物）的全部存活角色 |
| `{TargetType.SPREAD}` | 阵营锚点：散射锚点所在阵营的存活角色；hit_count 超过人数时每人至少一次，多余随机；否则纯随机 |

## source（系统填充，词缀可引用）

卡牌来源（生成/注入者）名称，由系统填充，无需输出。词缀可用「非 source 者」指代当前持有者（当其不等于 source 时），配合 `transferable` 实现「传给敌人后反噬对方」。"""
