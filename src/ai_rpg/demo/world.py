...
def create_single_hunter_blueprint(game_name: str) -> Blueprint:
    ...
    # 测试攻击力，打的快一点。
    actor_hunter.character_stats.attack = 20
    ...
    # 直接给 外观描述，减少一次推理生成。
    actor_hunter.character_sheet.appearance = """一名年近三十的男子，身形精悍灵活，肌肉线条结实分明，是常年山林活动留下的痕迹。
肤色呈健康的褐色，是风吹日晒的结果。他蓄着短须，头发简单束成发髻。眼神沉静而警觉。上身穿着轻便的短衫，由某种带有细密鳞纹的暗褐色皮革缝制，胸口和肩部有颜色略深的硬化皮革加固，腰间用麻绳束带系紧。
左肩从短衫领口边缘露出三道平行的、已愈合的淡白色疤痕。
右手虎口处有明显的厚茧。他背负着一把以山竹为材、刻有简单兽纹的长弓，弓弦紧绷。"""
