...
class McpState(TypedDict, total=False):
    ...
    llm: AzureChatOpenAI  # Azure OpenAI GPT实例，整个graph流程共享
...
async def _llm_invoke_node(state: McpState) -> McpState:
    """
    LLM调用节点：调用Azure OpenAI GPT生成响应
...
async def create_compiled_mcp_stage_graph(
    ...
    # 创建新的 AzureChatOpenAI 实例
    llm = create_azure_openai_gpt_llm()
    assert llm is not None, "AzureChatOpenAI instance is not available"
...
async def stream_mcp_graph_updates(
    ...
    if not llm_instance:
        ...
        llm_instance = create_azure_openai_gpt_llm()
