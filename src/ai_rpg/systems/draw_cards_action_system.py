...
**卡牌**是本回合的战斗结算单元，封装行动的完整信息（名称、描述、攻防数值、目标），将交由战斗系统进行战斗结算。
...
**描述**: 采用三段式结构化格式（必须包含段落标记）：

1. **【行动】** 第一人称描述具体动作和战术意图（1-2句话）
2. **【机制】** 明确声明本次行动的战斗规则效果（必填项）
   - 从技能/状态中提取战斗规则，用陈述句直接说明
   - 禁止使用"部分"、"可能"、"尝试"等模糊词汇
   - 示例：
     * "本次攻击无视目标的所有防御值"
     * "削弱目标防御力3点"
     * "本次行动获得+2攻击加成"
3. **【代价】** 若有风险或消耗则说明，无则省略此段
