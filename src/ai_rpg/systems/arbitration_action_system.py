...
**卡牌**封装完整行动信息（使用者、目标、攻防、描述）。按序结算，先手可影响后手。
...
1. **卡牌机制优先**：卡牌【机制】段落声明的特殊规则覆盖基础计算
2. **数值基数约束**：卡牌攻防值为计算基数，环境微调应基于场景互动，保持合理幅度
3. **死亡终止**：角色HP归零时立即失去行动能力，后续卡牌不执行
4. **场景互动**：设计环境互动，先手可改变环境影响后手

在combat_log中说明关键的环境影响或数值调整原因
