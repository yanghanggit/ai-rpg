...
函数命名规则：
    ...
    exhaust_card_game           — 指定角色消耗手牌（exhaust-card 命令）
    pass_turn_game              — 指定角色过牌（pass-turn 命令）
    use_consumable_item_game             — 使用消耗品（use-consumable-item 命令）
    ...
    equip_item_game             — 玩家装备物品（equip-item 命令）
"""
...
async def equip_item_game(
    ...
    """从存档复位，玩家装备指定物品（等同于终端命令 equip-item），并归档新状态。

    调用 activate_equip_item 添加玩家装备行动，然后驱动 home_pipeline.process()。
    本次 pipeline NPC 不主动推理，仅响应装备操作。

    前置条件：玩家必须处于家园模式；至少指定一个装备槽位（非全 None）。

    Args:
        world: 由 restore_world() 反序列化的世界数据。
        player_session: 由 restore_world() 反序列化的玩家会话。
        weapon: 武器槽目标物品全名（None 不更换；"" 脱掉；非空字符串装备该物品）。
        armor: 套装槽目标物品全名（None 不更换；"" 脱掉；非空字符串装备该物品）。
        accessory: 饰品槽目标物品全名（None 不更换；"" 脱掉；非空字符串装备该物品）。
        save_dir: 新存档写入目录。
    ...
    if not success:
        logger.error(
            f"激活装备行动失败: weapon={weapon!r} armor={armor!r} accessory={accessory!r}"
        )
        return terminal_game
...
async def exhaust_card_game(
    ...
    只有指定角色触发 ExhaustCardsAction；其他角色本次 pipeline 不消耗牌。
    若角色名或卡牌名不合法，提前返回不归档。
    ...
    if not terminal_game.current_dungeon.is_ongoing:
        logger.error("exhaust-card 只能在战斗进行中使用")
        return terminal_game

    last_round = terminal_game.current_dungeon.latest_round
    if last_round is None or last_round.is_completed:
        logger.error("exhaust-card 当前没有未完成的回合可供消耗")
        return terminal_game

    success, message = await activate_exhaust_card_specified(terminal_game, actor, card)
    if not success:
        logger.error(f"exhaust-card 失败: {message}")
        return terminal_game
...
###############################################################################
async def use_consumable_item_game(
    world: World,
    player_session: PlayerSession,
    actor: str,
    item: str,
    ...
    """从存档复位，让指定角色使用指定消耗品，并归档新状态。

    消耗 1 点 energy 并推进行动顺序。
    若角色名、物品名不合法或非当前行动者，提前返回不归档。
    ...
    Args:
        world: 由 restore_world() 反序列化的世界数据。
        player_session: 由 restore_world() 反序列化的玩家会话。
        actor: 使用者角色全名（如 角色.旅行者.无名氏）。
        item: 要使用的消耗品名称（须存在于该角色背包中）。
        targets: 目标名称列表，可为空。
        save_dir: 新存档写入目录。
    ...
    if not terminal_game.current_dungeon.is_ongoing:
        logger.error("use-consumable-item 只能在战斗进行中使用")
        return terminal_game

    last_round = terminal_game.current_dungeon.latest_round
    if last_round is None or last_round.is_completed:
        logger.error("use-consumable-item 当前没有未完成的回合可供使用消耗品")
        return terminal_game
    ...
    if not success:
        logger.error(f"use-consumable-item 失败: {message}")
        return terminal_game
