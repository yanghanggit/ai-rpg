...
def _generate_prompt(
    ...
    response_sample = ActorResponse(
        ...
        speak_actions={
            "场景内角色全名": "你要对该角色说的话。回答策略：1) 优先检查当前上下文（角色设定、场景信息、对话历史）是否有相关信息 2) 如果有就直接基于已知信息回答 3) 如果信息不足但应该知道，必须说'让我想想'，同时使用记忆检索，禁止在不确定的情况下直接给出答案 4) 如果确实不在知识范围内就说'我不知道'。严禁编造上下文中不存在的角色名、地点名、道具名等游戏实体。如果使用了记忆检索，本次回复只能是'让我想想'。避免对同一问题重复相同回复。禁止复述其他角色的话。（场景内其他角色会听见）",
        },
        whisper_actions={
            "场景内角色全名": "你要悄悄对该角色说的话（只有你和目标角色能听见）",
        },
        announce_actions="你要向所有人宣布的内容（所有角色都能听见）",
