...
def create_demo_game_world(game_name: str) -> Boot:
    ...
    # 设置角色的初始状态
    actor_warrior.kick_off_message += f"""\n注意:{actor_wizard.name} 是你的同伴。你战斗时喜欢先击晕敌人再发动攻击，这样可以造成更高的伤害。你也会架起剑势先观察敌人的攻击再进行反击，遇到危险时你唯一的保命手段就是用短剑限制敌人的行动然后与其拉开距离。你讨厌火元素的法术，因为你认为它们是邪恶的。如果你遇到使用火元素魔法的人，你会立刻将攻击目标变成这个人，就算这个人是你的同伴你也会毫不犹豫的攻击。"""
