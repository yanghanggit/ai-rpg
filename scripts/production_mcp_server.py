...
这是一个独立部署的 MCP 服务器进程，设计用于与 MCP 客户端（如 run_deepseek_mcp_chat.py）通信。
...
使用方法：
    ...
    python scripts/production_mcp_server.py --transport stdio
    
    # 启动 HTTP 模式（适合多客户端或 Web 集成）
    python scripts/production_mcp_server.py --transport streamable-http --port 8765
    
    # 启动 SSE 模式（适合 Web 应用）
    python scripts/production_mcp_server.py --transport sse --port 8766
"""
...
@app.tool()
async def text_processor(text: str, operation: str = "upper", **kwargs) -> str:
    """
    高级文本处理工具
...
@app.tool()
async def file_operations(operation: str, file_path: str = "", content: str = "", encoding: str = "utf-8") -> str:
    """
    安全的文件操作工具
    ...
    try:
        ...
        if operation == "read":
            ...
            return f"文件{'存在' if os.path.exists(file_path) else '不存在'}：{file_path}"
...
async def get_server_status() -> str:
    ...
    try:
        status = {
            "服务器配置": config.to_dict(),
            "运行状态": "正常",
            "可用工具数": len(app._tool_manager.tools),
            "可用资源数": len(app._resource_manager.resources),
            "可用提示数": len(app._prompt_manager.prompts),
            "内存使用": "未知（需要 psutil）",
            "连接状态": "活跃"
        }
...
async def get_recent_logs(count: str) -> str:
    ...
    try:
        ...
        return json.dumps({"logs": logs, "total": log_count}, ensure_ascii=False, indent=2)
    except ValueError:
        return "错误：无效的数字格式"
    except Exception as e:
        return f"错误：{str(e)}"
...
@app.prompt()
async def data_processing(task_type: str = "analysis", data_format: str = "json") -> types.GetPromptResult:
    """
    数据处理提示模板
...
@click.option(
    ...
    type=int,
    default=8765,
    help="端口号（仅适用于 sse 和 streamable-http）"
)
@click.option(
    "--host",
    default="127.0.0.1",
    help="主机地址（仅适用于 sse 和 streamable-http）"
)
@click.option(
    "--log-level",
...
def main(transport: str, port: int, host: str, log_level: str):
    """启动生产级 MCP 服务器"""
