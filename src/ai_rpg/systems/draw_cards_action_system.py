...
def _generate_draw_prompt(
    ...
    fields_line = (
        ...
        '- effects：状态触发标记列表。出牌后系统读取此字段，非空时启动独立 LLM 推理，为出牌者/目标生成实际 StatusEffect（持续性增减益）。每项格式"[名称]:触发倾向描述"（如"[燃烧]:可能引发持续火焰伤害"、"[中毒]:持续造成毒素伤害"）；仅代表可能触发的状态效果倾向，并非立即生效数值。纯即时伤害无持续影响时输出 []\n'
        "- playable：布尔值。是否允许出牌；默认 true，封印/禁出场景填 false\n"
        "- discardable：布尔值。是否允许弃牌；默认 true，禁弃场景填 false\n"
        "- damage_dealt：单次攻击造成的伤害值（基于攻击力合理推算，整数）\n"
        "- hit_count：攻击次数（默认 1；多段攻击如回旋镖可设为 2~4）\n"
