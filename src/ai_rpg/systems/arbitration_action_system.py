...
```json
{{
  "combat_log": "【角色使用卡牌 → 环境互动(含数值) → 伤害结果(更新HP) → 卡牌代价】 → 最后所有角色最终HP(角色.HP=X/Y) → 环境更新。所有效果明确数值 → 精简紧凑文本",
  "narrative": "将战斗过程故事化简短概括：角色行动→环境响应→影响结果→更新后的环境状态。禁用数字，使用感官描写，精简紧凑文本"
}}
```

**约束规则**
- combat_log压缩优化： 角色名简写(仅保留最后一段) → 删除所有动作描写 → 卡牌仅写所使用的卡牌名 → 环境互动仅写[物体名+效果+数值] → **禁止出现任何计算公式和数学符号，仅输出最终计算结果的数值** → 状态/代价仅使用[状态/代价名+效果+数值]  → 删除"→"外所有连接词和修饰词
