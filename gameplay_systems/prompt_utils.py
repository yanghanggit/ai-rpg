...
格式示例：@目标角色(的全名);/技能道具(全名);/配置道具1(全名)=消耗数量;/配置道具2(全名)=消耗数量;...
### 解释与规则：
- 注意其中的符号：@、/、=、;。均要严格遵循。
- 指令含义：表示经过思考后，准备对目标使用技能，并选择配置道具及其消耗数量以实现目标。
    - 技能道具与配置道具：都必须是角色拥有的道具。
- 技能道具：只能选择一个。
- 配置道具：可以根据需要，使用多个或不使用。
    - 如果是'武器'类型道具，单次只能配置一个武器。
    - 如果是'消耗品'类型道具，技能施放成功后，系统会扣除相应数量(降至0时系统会自动移除)。
- 消耗数量：如使用配置道具，消耗数量必须至少为1(即填写 /配置道具(全名)=1)
- 目标选择：必须根据技能描述选择至少一个目标。"""
