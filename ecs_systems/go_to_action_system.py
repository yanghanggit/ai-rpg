...
class GoToActionHelper:
    ...
    def __init__(self, context: RPGEntitasContext, entity: Entity, target_stage_name: str) -> None:

        self._context: RPGEntitasContext = context
        self._entity: Entity = entity
        self._current_stage_name: str = cast(ActorComponent, entity.get(ActorComponent)).current_stage
...
class ActorLeaveStageEvent(IStageDirectorEvent):
    ...
    def __init__(self, actor_name: str, current_stage_name: str, target_stage_name: str) -> None:
...
class ActorEnterStageEvent(IStageDirectorEvent):
    ...
    def __init__(self, actor_name: str, target_stage_name: str, last_stage_name: str) -> None:
...
###############################################################################################################################################            
    def enter_target_stage(self, helper: GoToActionHelper) -> None:
        ...
        #进入场景的事件需要通知相关的人
        enter_stage_event = ActorEnterStageEvent(actor_comp.name, helper._target_stage_name, helper._current_stage_name)
...
###############################################################################################################################################
    def before_leave_current_stage(self, helper: GoToActionHelper) -> None:
        #目前就是强行刷一下history
        StageDirectorSystem.director_events_to_actor(self._context, helper._entity, None)
        StageDirectorSystem.director_events_to_player(self._context, helper._entity, None)
###############################################################################################################################################
    def leave_current_stage(self, helper: GoToActionHelper) -> None:
        ...
        StageDirectorComponent.add_event_to_stage_director(self._context, 
                                                           helper._entity, 
                                                           ActorLeaveStageEvent(actor_comp.name, helper._current_stage_name, helper._target_stage_name))
