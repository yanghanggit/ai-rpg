...
from ..models import ConsumableItem, MaterialItem
...
class CraftConsumableItemScreen(BaseGameScreen):
    ...
    def on_mount(self) -> None:
        ...
        log.write(CRAFT_CONSUMABLE_HEADER)
        self._load_data()
    ...
    async def _load_data(self) -> None:
        ...
        try:
            ...
            logger.warning(
                f"CraftConsumableItemScreen._load_data: 加载材料失败 error={e}"
            )
            log.write(f"[yellow]⚠ 加载材料列表失败: {e}[/]")
            return

        logger.info(
            f"CraftConsumableItemScreen._load_data: materials={self._material_list}"
        )
        self._show_material_list()
    ...
    async def _do_craft(self) -> None:
        ...
        try:
            ...
            logger.error(f"CraftConsumableItemScreen._do_craft: 请求失败 error={e}")
        ...
        try:
            ...
            logger.info(
                f"CraftConsumableItemScreen._do_craft: 任务完成 task_id={task_id}"
            )
            await self._show_craft_result(log)
        except TaskFailedError as e:
            log.write(f"[bold red]❌ 合成失败: {e}[/]")
            logger.error(
                f"CraftConsumableItemScreen._do_craft: 任务失败 task_id={task_id} error={e}"
            )
        except TimeoutError:
            log.write("[bold yellow]⚠️ 合成超时，请检查服务器状态[/]")
            logger.warning(
                f"CraftConsumableItemScreen._do_craft: 任务轮询超时 task_id={task_id}"
            )
    ...
    async def _show_craft_result(self, log: RichLog) -> None:
        ...
        try:
            ...
            logger.warning(
                f"CraftConsumableItemScreen._show_craft_result: 获取储物箱失败 error={e}"
            )
        ...
        for item in storage_component.items:
            ...
            if item.on_use_prompt:
                log.write(f"  [cyan]效果提示[/]：{item.on_use_prompt[0]}")
            log.write("")
