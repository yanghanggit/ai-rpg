"""CombatRoundEndSettlementSystem 单元测试。"""
...
@pytest.fixture()
def system(game: MagicMock) -> CombatRoundEndSettlementSystem:
    return CombatRoundEndSettlementSystem(game)
...
def test_apply_forwards_add_effect_affixes(
    ...
    system._apply_round_end_effect_response(
        _make_client(
            "英雄",
            '{"hp": 17, "combat_log": "感染扩散", "add_effect_affixes": ["感染繁殖：滋生新病灶"]}',
        )
    )

    args, _ = mock_accumulate.call_args
    assert args[0] is entity
    triggers = args[1]
    assert len(triggers) == 1
    assert triggers[0].source == "回合末结算"
    assert triggers[0].affix == "感染繁殖：滋生新病灶"
...
async def test_execute_skips_when_not_ongoing(
    game: MagicMock, system: CombatRoundEndSettlementSystem
) -> None:
