...
2. **技能对自身的限制**：每个技能使用后必须对使用者产生一个限制状态：  
   - 眩晕：无法行动  
   - 沉默：无法使用法术类技能  
   - 力竭：体力透支，无法防御  
...
- 第一局的status_effects根据角色进入战斗时的环境，内心活动，和其他角色的状态生成，而不是技能里提到的状态
...
- 技能效果格式：主要效果 + 可选状态效果 + 自身限制状态
- 技能的description和effect里禁止包含角色名称
- status_effects根据角色上回合行动时受到的其他角色的技能状态效果和自身技能的限制状态生成，而不是这回合生成的技能里提到的状态
- 同一时间可以存在多个status_effects
- 使用有趣、意想不到的风格描述效果产生的原因
...
- 禁用换行/空行
- 请严格按照示例格式输出合规JSON
- 输出格式要求中的任何数字都不是实际值，请根据你的当前状态生成新的值"""
