...
def _generate_prompt(
    ...
    response_sample = ActorResponse(
        speak_actions={
            "场景内角色全名": "你要说的内容。1）如果是回答问题，根据自己的上下文中已有的信息来进行答复。2）如果上下文中的信息没有关于这个问题的准确答案，就回答“让我想想”。3）如果上一次的speak_actions里已经出现了“让我想想”，那么这次的speak_actions必须是直接回答问题，不能再出现“让我想想”。并且这次回答问题时，如果上下文中有问题的相关信息或准确答案就直接回答，如果上下文中还是没有相关信息或准确答案就回答“我不知道”。4）但如果上一次的speak_actions里已经出现了“我不知道”，那么这次的speak_actions禁止再出现“我不知道”，而是一定会根据这次的问题重新思考来决定说“让我想想”还是直接回答问题。5）如果不是回答问题，则根据上下文，mind_voice_actions和搜索到的信息来决定如何回复对话。注意：只关注本次的提问，对于之前的提问，不需要再次回答。speak_actions里禁止复述其他角色的话（场景内其他角色会听见）",
        },
        whisper_actions={
        ...
        query_actions="你要说的内容。判断逻辑：1) 如果在本次对话中有人向你提问，且你在本次的speak_actions中回复了“让我想想”，则这次的query_actions必须是根据上下文和这次的提问来决定如何发问从而深度回忆自己的记忆去获取相关的信息和答案。2）如果没有最新的提问，则根据对话和上下文来决定是否需要发问以查询记忆中与对话中有关的信息。如果发问了，就以此为参考进行回复。如果没有发问，则可以返回‘ ’。注意：1）如果发问，则只关注其他角色本次的提问和对话，对于之前的提问，严禁再次思考。2）所有的query_actions必须精准，简短，发问时只包含最关键的问题，不要带有多余的信息。（只有你自己能听见，这个行动会触发记忆查询）",
    )
...
- query_actions默认不触发，但是如果speak_actions里已经出现了“让我想想”，那么这次的query_actions一定会触发。如果对话里有信息你不知道或者你感兴趣，也会触发query_actions。
