...
from auxiliary.prompt_maker import broadcast_action_prompt, speak_action_prompt, __unique_prop_taken_away__
...
class SearchFailedEvent(DirectorEvent):
    ...
    def convert(self, targetname: str, extended_context: ExtendedContext) -> str:
        ...
        log = __unique_prop_taken_away__(self.who_search_failed, self.target)
        logger.info(log)
        return log
####################################################################################################################################
####################################################################################################################################
