...
- **on_hit_affixes**：出牌命中目标时触发的延迟词缀列表，格式同上（如 `[撕裂伤]:命中后可能引发持续流血`）；凡涉及"下回合/持续N回合"等跨回合影响的效果必须放在这里；无命中效果时输出 []
- **modifiers**：即时修正词缀列表，格式 `[名称]:即时修正描述`，仅描述本次仲裁计算内立即生效的一次性数值修正（如 `[穿甲]:无视目标防御的一部分`）；严禁出现"下回合/持续N回合"等跨回合表述；无即时修正时输出 []
