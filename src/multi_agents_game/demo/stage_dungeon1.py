...
def create_stage_cave1() -> Stage:
    ...
    return create_stage(
        ...
        stage_profile="你是一个黑暗干燥的洞窟，地上都是易燃的干草，洞窟深处似乎有哥布林在进行某种活动。",
        actors=[],
    )
...
def create_demo_dungeon1() -> Dungeon:
    ...
    actor_goblin.kick_off_message += f"""\n注意:你的技能生成模式是第一种生成模式。你非常狡猾，所以身上带了一件哥布林的传家宝项链用来保命，这个项链会让你在死亡时以百分之十的血量复活，并且复活后的第一次攻击会造成双倍伤害。但是这个项链只能让你复活一次。项链属于装备，不会在卡牌中出现，死亡时会自动触发，不受负面效果影响，不占用行动回合。"""
