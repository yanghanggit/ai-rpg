...
class ReplicateTextToImage:
    ...
    def get_available_models(self, tag: str = "text-to-image") -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for model in models:
                    ...
                    ) or any("image" in tag for tag in tags):
                        text_to_image_models.append(
                            {
                                ...
                                "latest_version": (
                                    model.get("latest_version", {}).get("id")
    ...
    def generate_image(
        ...
        if model_name not in self.MODELS:
            raise ValueError(
                f"不支持的模型: {model_name}. 可用模型: {list(self.MODELS.keys())}"
            )
    ...
    def generate_and_download(
        self, prompt: str, output_dir: str = "generated_images", **kwargs: Any
    ) -> str:
        """
        生成并下载图片的便捷方法
...
def main() -> None:
    ...
    parser.add_argument(
        "--discover-models", action="store_true", help="从API发现新的文生图模型"
    )
