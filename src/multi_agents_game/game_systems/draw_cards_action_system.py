...
def _generate_prompt(
    ...
    return f"""# 请你生成 {skill_creation_count} 个技能，一个技能是防御或攻击类型的技能，受角色的属性影响。另一个技能是观察，角色观察周围环境来查看是否有可以利用的地方，如果有，就立即生效（注意，环境利用是指通过利用环境中的物品，产生的对敌方的间接的攻击或施加负面效果行为，或者是对己方的防御或施加增益效果的行为。同一环境物体在同一回合内不可重复利用，如果已有角色使用该物体，下一角色将不能利用该环境物体），此技能不受角色属性影响。（注意，环境利用主要是和环境中的物体交互而产生的影响敌人或己方的负面或正面效果）并决定如何使用。
## 当前场景状态
