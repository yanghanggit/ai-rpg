...
def show_current_data(df: pd.DataFrame, data_type: str):
    """显示当前数据"""
    print(f"\n=== 当前{data_type}数据 ===")
...
def manage_data(file_path: str, data_type: str):
    """管理数据的主要函数"""
    # 读取数据
    ...
    if df is None:
        ...
        if data_type == "地牢":
            ...
            df = pd.DataFrame(columns=["name", "character_sheet_name", "actor_profile", "appearance"])
    ...
    while True:
        ...
        if choice == '1':
            ...
            if modified and input("有未保存的修改，确认放弃? (y/N): ").lower() not in ['y', 'yes']:
                continue
            return
        else:
            print("请输入1-5")
