...
本文件包含所有游戏动作的 async def 实现函数，以及日志初始化工具。
CLI 入口（click 命令）位于 run_agent_game.py，由其 import 本文件的所有符号。

函数命名规则：
    _create_and_initialize_game  — 创建新游戏（new 命令）
    _restore_game                — 从存档复位 TCGGame（各命令共享入口）
    _advance_game                — 家园推进（advance 命令）
    _speak_game                  — 玩家对话（speak 命令）
    _switch_stage_game           — 场景切换（switch-stage 命令）
    _enter_dungeon_game          — 进入地下城（enter-dungeon 命令）
    _draw_cards_game             — 抽牌（draw-cards 命令）
    _play_cards_specified_game   — 指定角色出牌（play-cards-specified 命令）
    _exit_dungeon_and_return_home_game — 退出地下城（exit-dungeon 命令）
    _next_dungeon_game           — 进入下一关（next-dungeon 命令）
    _retreat_game                — 主动撤退（retreat 命令）
    _generate_dungeon_game       — 生成地下城（generate-dungeon 命令）
"""
