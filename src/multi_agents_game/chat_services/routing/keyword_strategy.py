...
from .route_strategy import RouteStrategy, RouteDecision
...
class KeywordRouteStrategy(RouteStrategy):
    ...
    def should_route_to_rag(self, query: str, context: Optional[Dict[str, Any]] = None) -> RouteDecision:
        """基于关键词匹配进行路由决策"""
        ...
        # 计算匹配度
        match_ratio = len(matched_keywords) / len(self.keywords) if self.keywords else 0.0
