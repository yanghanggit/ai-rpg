...
def _generate_prompt(
    ...
    response_sample = ActorResponse(
        speak_actions={
            "场景内角色全名": "你要说的内容。1）如果是回答问题，根据自己的上下文中已有的信息来进行答复。2）如果上下文中的信息没有关于这个问题的准确答案，就回答“让我想想”。3）如果上一次的speak_actions里已经出现了“让我想想”，那么这次speak_actions必须是直接回答问题，不能再出现“让我想想”。并且这次回答问题时，如果上下文中有问题的准确答案就直接回答，如果上下文中还是没有相关信息或准确答案就回答“我不知道”。4）但如果上一次的speak_actions里已经出现了“我不知道”，那么这次的speak_actions禁止再出现“我不知道”，而是一定会根据这次的问题重新思考来决定说“让我想想”还是直接回答问题。注意：只关注本次的提问，对于之前的提问，不需要再次回答。（场景内其他角色会听见）",
        },
        whisper_actions={
